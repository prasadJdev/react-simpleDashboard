...
export default function Products() {
  ...
  useEffect(() => {
    ...
    if (storedCategories)
      dispatch(loadStoredCategoriesDataToState(storedCategories));
  }, []);

  // Sorting
  const sortSelected = useRef();

  const SORTACTIONS = {
    PRICEASCENDING: "priceAscending",
    PRICEDESENDING: "priceDescending",
    NAMEATOZ: "nameAZ",
    NAMEZTOZ: "nameZA",
  };

  let sortedData = [];
  sortedData = products.map((product)=>{
    return product
  })
  const sortData = () => {
    const selectedSortOrder = sortSelected.current.value;
    // console.log("Jello");
    // console.log(sortedData);
    switch (selectedSortOrder) {
      case SORTACTIONS.PRICEASCENDING:
        sortedData.sort((a, b) => ((parseInt(a.price) > parseInt(b.price)) ? 1 : (parseInt(a.price) < parseInt(b.price)) ? -1 : 0 ));
        // console.log(sortedData);
        dispatch(loadStoredProductsDataToState(sortedData))
        break;
        case SORTACTIONS.PRICEDESENDING:
          sortedData.sort((a, b) => ((parseInt(a.price) < parseInt(b.price)) ? 1 : (parseInt(a.price) > parseInt(b.price)) ? -1 : 0 ));
          // console.log(sortedData);
          dispatch(loadStoredProductsDataToState(sortedData))
          break;
          ...
          case SORTACTIONS.NAMEATOZ:
            sortedData.sort((a, b) => ((a.name > b.name) ? 1 : (a.name < b.name) ? -1 : 0 ));
            // console.log(sortedData);
            dispatch(loadStoredProductsDataToState(sortedData))
            break;
            case SORTACTIONS.NAMEZTOZ:
              sortedData.sort((a, b) => ((a.name < b.name) ? 1 : (a.name > b.name) ? -1 : 0 ));
              // console.log(sortedData);
              dispatch(loadStoredProductsDataToState(sortedData))
              break;
    ...
    <>
      ...
      <div className="sortFilterContainer">
        <div className="Sorting">
          <h5>Sort</h5>
          <Form.Select ref={sortSelected} onChange={sortData} size="sm">
            <option selected value={SORTACTIONS.PRICEASCENDING}>
              Low -&gt; High
            </option>
            <option value={SORTACTIONS.PRICEDESENDING}>High -&gt; Low</option>
            <option value={SORTACTIONS.NAMEATOZ}>A -&gt; Z</option>
            <option value={SORTACTIONS.NAMEZTOZ}>Z -&gt; A</option>
          </Form.Select>
        </div>
